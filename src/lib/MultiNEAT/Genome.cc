...
#include <MultiNEAT/Genome.hh>
#include <MultiNEAT/Parameters.hh>
...
// forward
ActivationFunction GetRandomActivation(const Parameters &a_Parameters,
                                       RNG &a_RNG);
...
Genome::Genome(
    unsigned int a_ID, unsigned int a_NumInputs,
    unsigned int a_NumHidden, // ignored for seed type == 0, specifies number of
                              // hidden units if seed type == 1
  ...
  if (a_Parameters.DontUseBiasNeuron == false) {
    ...
    // The order of the neurons is very important. It is the following: INPUTS,
    // BIAS, OUTPUTS, HIDDEN ... (no limit)
  ...
  for (unsigned int i = 0; i < (a_NumOutputs); i++) {
    ...
    t_ngene.Init(
        (a_Parameters.MinActivationA + a_Parameters.MaxActivationA) / 2.0f,
  ...
  // Fully connect every neuron to every other. Only inputs don't receive
  // output.
  for (unsigned int i = a_NumInputs;
       i < (a_NumInputs + a_NumOutputs + a_NumHidden); i++) {
    for (unsigned int j = 0; j < (a_NumInputs + a_NumOutputs + a_NumHidden);
         j++) {
      // add the link
...
Genome::Genome(
    ...
    unsigned int a_NumLayers =
        1) // number of hidden layers. Each will have a_NumHidden nodes
{
  ASSERT((a_NumInputs > 1) && (a_NumOutputs > 0));
  RNG t_RNG;
  t_RNG.TimeSeed();
    ...
    if (!a_FS_NEAT) {
      ...
      for (unsigned int n = 0; n < a_NumLayers; n++) {
        ...
        for (unsigned int i = 0; i < a_NumHidden; i++) {
          for (unsigned int j = 0; j < prev_layer_size; j++) {
            ...
            LinkGene l = LinkGene(j + last_src_id, i + last_dest_id, t_innovnum,
                                  0.0, false);
            l.InitTraits(a_Parameters.LinkTraits, t_RNG);
            m_LinkGenes.push_back(l);
            t_innovnum++;
      ...
      /*if (a_Parameters.DontUseBiasNeuron == false)
      {
          ...
          for (unsigned int i = 0; i < a_NumOutputs; i++)
          {
              // add the link
              // created with zero weights. needs future random initialization.
      !!!!!!!! LinkGene l = LinkGene(a_NumInputs, i + last_dest_id, t_innovnum,
      0.0, false); l.InitTraits(a_Parameters.LinkTraits, t_RNG);
              m_LinkGenes.push_back(l);
              t_innovnum++;
          }
      }*/
    }
  } else // The links connecting every input to every output - perceptron
         // structure
  {
    if ((!a_FS_NEAT) && (a_SeedType == 0)) {
      for (unsigned int i = 0; i < (a_NumOutputs); i++) {
        for (unsigned int j = 0; j < a_NumInputs; j++) {
          ...
          // !!!!!!!!
          LinkGene l =
              LinkGene(j + 1, i + a_NumInputs + 1, t_innovnum, 0.0, false);
          l.InitTraits(a_Parameters.LinkTraits, t_RNG);
          m_LinkGenes.push_back(l);
          t_innovnum++;
        }
...
void Genome::SetID(unsigned int a_id) { m_ID = a_id; }

unsigned int Genome::GetID() const { return m_ID; }
...
bool Genome::HasLoops() {
  ...
  for (int i = 0; i < net.m_connections.size(); i++) {
    bs::add_edge(net.m_connections[i].m_source_neuron_idx,
                 net.m_connections[i].m_target_neuron_idx, g);
  }
...
void Genome::BuildPhenotype(NeuralNetwork &a_Net) {
  ...
  for (unsigned int i = 0; i < NumLinks(); i++) {
    ...
    if (m_LinkGenes[i].m_Traits.count("hebb_rate") == 1) {
      try {
        t_c.m_hebb_rate =
            boost::get<double>(m_LinkGenes[i].m_Traits["hebb_rate"].value);
    ...
    if (m_LinkGenes[i].m_Traits.count("hebb_pre_rate") == 1) {
      try {
        t_c.m_hebb_pre_rate =
            boost::get<double>(m_LinkGenes[i].m_Traits["hebb_pre_rate"].value);
      } catch (std::exception e) {
        // do nothing
      }
    }
...
// The procedure uses the [0] CPPN output for creating nodes, and if the
// substrate is leaky, [1] and [2] for time constants and biases Also assumes
...
void Genome::BuildHyperNEATPhenotype(NeuralNetwork &net, Substrate &subst) {
  ...
  net.SetInputOutputDimentions(
      static_cast<unsigned short>(subst.m_input_coords.size()),
  ...
  // now loop over every potential connection in the substrate and take its
  // weight
  ...
  if (subst.m_leaky) {
    for (unsigned int i = net.NumInputs(); i < net.m_neurons.size(); i++) {
      ...
      for (unsigned int n = 0; n < net.m_neurons[i].m_substrate_coords.size();
           n++) {
      ...
      Scale(t_bias, -1, 1, -subst.m_max_weight_and_bias,
            subst.m_max_weight_and_bias);
  ...
  if (subst.m_custom_connectivity.size() == 0) {
    ...
    for (int i = net.NumInputs(); i < net.m_neurons.size(); i++) {
      ...
      for (int j = 0; j < net.m_neurons.size(); j++) {
        ...
        if (((!subst.m_allow_input_hidden_links) &&
             ((net.m_neurons[j].m_type == INPUT) &&
              (net.m_neurons[i].m_type == HIDDEN)))

            || ((!subst.m_allow_input_output_links) &&
                ((net.m_neurons[j].m_type == INPUT) &&
                 (net.m_neurons[i].m_type == OUTPUT)))

            || ((!subst.m_allow_hidden_hidden_links) &&
                ((net.m_neurons[j].m_type == HIDDEN) &&
                 (net.m_neurons[i].m_type == HIDDEN) && (i != j)))
            ...
            || ((!subst.m_allow_output_output_links) &&
                ((net.m_neurons[j].m_type == OUTPUT) &&
                 (net.m_neurons[i].m_type == OUTPUT) && (i != j)))
    ...
    for (unsigned int idx = 0; idx < subst.m_custom_connectivity.size();
         idx++) {
      ...
      if ((src_type == INPUT) || (src_type == BIAS)) {
        ...
        j = subst.m_input_coords.size() + subst.m_output_coords.size() +
            src_idx;
      ...
      if ((dst_type == INPUT) || (dst_type == BIAS)) {
        ...
        i = subst.m_input_coords.size() + subst.m_output_coords.size() +
            dst_idx;
  ...
  for (unsigned int conn = 0; conn < t_to_query.size(); conn++) {
    ...
    if (((t_link > 0) && (!subst.m_query_weights_only)) ||
        (subst.m_query_weights_only)) {
...
double Genome::CompatibilityDistance(Genome &a_G, Parameters &a_Parameters) {
  ...
  t_genome_link_trait_difference =
      m_GenomeGene.GetTraitDistances(a_G.m_GenomeGene.m_Traits);

  // used for percentage of excess/disjoint genes calculation
  int t_max_genome_size = static_cast<int>(NumLinks() < a_G.NumLinks())
                              ? (a_G.NumLinks())
                              : (NumLinks());
  int t_max_neurons = static_cast<int>(NumNeurons() < a_G.NumNeurons())
                          ? (a_G.NumNeurons())
                          : (NumNeurons());
  ...
  while (!((t_g1 == m_LinkGenes.end()) && ((t_g2 == a_G.m_LinkGenes.end())))) {
    ...
    // end of second genome?
    {
      ...
      if (t_g1innov == t_g2innov) {
        ...
        std::map<std::string, double> link_trait_difference =
            t_g1->GetTraitDistances(t_g2->m_Traits);
        // add to the totals
        for (auto it = link_trait_difference.begin();
             it != link_trait_difference.end(); it++) {
  ...
  for (unsigned int i = 0; i < NumNeurons(); i++) {
    ...
    if ((m_NeuronGenes[i].Type() != INPUT) &&
        (m_NeuronGenes[i].Type() != BIAS)) {
      // a match
      if (a_G.HasNeuronID(m_NeuronGenes[i].ID())) {
        t_num_matching_neurons++;

        double t_A_difference =
            m_NeuronGenes[i].m_A - a_G.GetNeuronByID(m_NeuronGenes[i].ID()).m_A;
        if (t_A_difference < 0.0f)
          t_A_difference = -t_A_difference;
        t_total_A_difference += t_A_difference;

        double t_B_difference =
            m_NeuronGenes[i].m_B - a_G.GetNeuronByID(m_NeuronGenes[i].ID()).m_B;
        ...
        double t_bias_difference =
            ...
            a_G.GetNeuronByID(m_NeuronGenes[i].ID()).m_Bias;
        ...
        if (m_NeuronGenes[i].m_ActFunction !=
            a_G.GetNeuronByID(m_NeuronGenes[i].ID()).m_ActFunction) {
        ...
        std::map<std::string, double> neuron_trait_difference =
            m_NeuronGenes[i].GetTraitDistances(
                a_G.GetNeuronByID(m_NeuronGenes[i].ID()).m_Traits);
        // add to the totals
        for (auto it = neuron_trait_difference.begin();
             it != neuron_trait_difference.end(); it++) {
  ...
  t_total_distance =
      ...
      (a_Parameters.WeightDiffCoeff *
       (t_total_weight_difference / t_num_matching_links)) +
      (a_Parameters.ActivationADiffCoeff *
       (t_total_A_difference / t_num_matching_neurons)) +
      (a_Parameters.ActivationBDiffCoeff *
       (t_total_B_difference / t_num_matching_neurons)) +
      (a_Parameters.TimeConstantDiffCoeff *
       (t_total_timeconstant_difference / t_num_matching_neurons)) +
      (a_Parameters.BiasDiffCoeff *
       (t_total_bias_difference / t_num_matching_neurons)) +
  ...
  for (auto it = t_total_link_trait_difference.begin();
       it != t_total_link_trait_difference.end(); it++) {
    t_total_distance +=
        (a_Parameters.LinkTraits[it->first].m_ImportanceCoeff * it->second) /
        t_num_matching_links;
  }
  for (auto it = t_total_neuron_trait_difference.begin();
       it != t_total_neuron_trait_difference.end(); it++) {
    t_total_distance +=
        (a_Parameters.NeuronTraits[it->first].m_ImportanceCoeff * it->second) /
        t_num_matching_neurons;
  }
  for (auto it = t_genome_link_trait_difference.begin();
       it != t_genome_link_trait_difference.end(); it++) {
    t_total_distance +=
        (a_Parameters.GenomeTraits[it->first].m_ImportanceCoeff * it->second);
  }
...
// Returns a random activation function from the canonical set based ot
// probabilities
ActivationFunction GetRandomActivation(const Parameters &a_Parameters,
                                       RNG &a_RNG) {
...
bool Genome::Mutate_AddNeuron(InnovationDatabase &a_Innovs,
  ...
  // Check if an innovation of this type already occured somewhere in the
  // population
  ...
  if (t_innovid == -1) {
    ...
    // Adjust the SplitY
    double t_sy = m_NeuronGenes[GetNeuronIndex(t_in)].SplitY() +
                  m_NeuronGenes[GetNeuronIndex(t_out)].SplitY();
    t_sy /= 2.0;
    ...
    Scale(t_TC, 0, 1, a_Parameters.MinNeuronTimeConstant,
          a_Parameters.MaxNeuronTimeConstant);
    // Scale(t_Bs, 0, 1, a_Parameters.MinNeuronBias,
    // a_Parameters.MaxNeuronBias);
    ...
    Clamp(t_TC, a_Parameters.MinNeuronTimeConstant,
          a_Parameters.MaxNeuronTimeConstant);
    Clamp(t_Bs, a_Parameters.MinNeuronBias, a_Parameters.MaxNeuronBias);

    // Initialize the neuron gene's properties
    t_ngene.Init(t_A, t_B, t_TC, t_Bs,
    ...
    // Second link
    LinkGene l2 =
        LinkGene(t_nid, t_out, t_l2id, t_orig_weight, t_recurrentflag);
    // Init the link's traits
    ...
    std::vector<int> t_idxs =
        a_Innovs.CheckAllInnovations(t_in, t_out, NEW_NEURON);
    bool t_found = false;
    ...
    Scale(t_TC, 0, 1, a_Parameters.MinNeuronTimeConstant,
          a_Parameters.MaxNeuronTimeConstant);
    // Scale(t_Bs, 0, 1, GlobalParameters.MinNeuronBias,
    // GlobalParameters.MaxNeuronBias);
...
bool Genome::Mutate_AddLink(InnovationDatabase &a_Innovs,
  ...
  // It may be the same neuron - this means that the connection is a looped
  // recurrent one. These are indexes in the NeuronGenes array!
  ...
  // if not recurrent, there is a probability that this link will be from the
  // bias if such link doesn't already exist. in case such link exists, search
  // for a standard feed-forward connection place
  else {
  ...
  if (!t_MakeRecurrent) {
    ...
    t_n1idx =
        static_cast<int>(NumInputs() - 1); // the bias is always the last input
    // try to find a neuron that is not connected to the bias already
    t_NumTries = 0;
    do {
      t_n2idx =
          a_RNG.RandInt(t_first_noninput, static_cast<int>(NumNeurons() - 1));
      t_NumTries++;
    ...
    } while ((HasLink(m_NeuronGenes[t_n1idx].ID(),
                      m_NeuronGenes[t_n2idx].ID()))); // already present?
    ...
    // otherwise continue trying to find a normal forward link
      ...
      } while ((m_NeuronGenes[t_n1idx].SplitY() >
                m_NeuronGenes[t_n2idx].SplitY()) // backward?
               || (HasLink(m_NeuronGenes[t_n1idx].ID(),
                           m_NeuronGenes[t_n2idx].ID())) // already present?
               || (m_NeuronGenes[t_n1idx].Type() ==
                   OUTPUT) // consider connections out of outputs recurrent
  ...
  // find a good pair of nodes for a recurrent link (non-looped)
    ...
    // NOTE: this considers output-output connections as forward. Should be
    // fixed.
    while ((m_NeuronGenes[t_n1idx].SplitY() <=
            m_NeuronGenes[t_n2idx].SplitY()) // forward?
           || (HasLink(m_NeuronGenes[t_n1idx].ID(),
                       m_NeuronGenes[t_n2idx].ID())) // already present?
    ...
    do {
      ...
      if (t_NumTries >= a_Parameters.LinkTries) {
        ...
        return false;
                      ...
                      m_NeuronGenes[t_n2idx].ID())) // already present?
                                                    //||
             //(m_NeuronGenes[t_n1idx].Type() == OUTPUT) // do not allow looped
             // recurrent on the outputs (experimental)
    );
  ...
  ASSERT((!HasLink(m_NeuronGenes[t_n1idx].ID(),
                   m_NeuronGenes[t_n2idx].ID()))); // already present?
  ...
  // So we have a good pair of neurons to connect. See the innovation database
...
// A cleanup procedure is invoked so any dead-ends or stranded neurons are also
// deleted returns true if succesful
bool Genome::Mutate_RemoveLink(RNG &a_RNG) {
  ...
  int t_link_index = static_cast<int>(
      t_randnum *
      static_cast<double>(NumLinks() -
...
bool Genome::Mutate_RemoveSimpleNeuron(InnovationDatabase &a_Innovs,
  ...
  for (unsigned int i = 0; i < NumLinks(); i++) {
    if (m_LinkGenes[i].ToNeuronID() ==
        m_NeuronGenes[t_neurons_to_delete[t_choice]].ID()) {
      t_l1idx = i;
      break;
    }
  }
  ...
  // OK now see if a link connecting the original 2 nodes is present. If it is,
  // we will just delete the neuron and quit.
  if (HasLink(m_LinkGenes[t_l1idx].FromNeuronID(),
              m_LinkGenes[t_l2idx].ToNeuronID())) {
    RemoveNeuronGene(m_NeuronGenes[t_neurons_to_delete[t_choice]].ID());
    return true;
  }
  // Else the link is not present and we will replace the neuron and 2 links
  // with one link
  else {
    ...
    int t_innovid =
        a_Innovs.CheckInnovation(m_LinkGenes[t_l1idx].FromNeuronID(),
    ...
    if (t_innovid == -1) {
      ...
      int t_newinnov =
          a_Innovs.AddLinkInnovation(m_LinkGenes[t_l1idx].FromNeuronID(),
                                     m_LinkGenes[t_l2idx].ToNeuronID());
      m_LinkGenes.push_back(LinkGene(m_LinkGenes[t_l1idx].FromNeuronID(),
                                     m_LinkGenes[t_l2idx].ToNeuronID(),
                                     t_newinnov, t_weight, false));
...
bool Genome::Mutate_LinkWeights(const Parameters &a_Parameters, RNG &a_RNG) {
  ...
  for (unsigned int i = 0; i < m_LinkGenes.size(); i++) {
    ...
    if ((!t_severe_mutation) &&
        (a_RNG.RandFloat() < a_Parameters.WeightMutationRate)) {
      ...
      if (ontail || (a_RNG.RandFloat() < a_Parameters.WeightReplacementRate)) {
        ...
        t_LinkGenesWeight +=
            a_RNG.RandFloatSigned() * a_Parameters.WeightMutationMaxPower;
      }
...
void Genome::Randomize_LinkWeights(double a_Range, RNG &a_RNG) {
  // For all links..
  for (unsigned int i = 0; i < NumLinks(); i++) {
    m_LinkGenes[i].SetWeight(a_RNG.RandFloatSigned() * a_Range);
  }
}
...
bool Genome::Mutate_NeuronActivations_A(const Parameters &a_Parameters,
                                        RNG &a_RNG) {
  // for all neurons..
  for (unsigned int i = 0; i < NumNeurons(); i++) {
    ...
    if ((m_NeuronGenes[i].Type() != INPUT) &&
        (m_NeuronGenes[i].Type() != BIAS)) {
      double t_randnum =
          a_RNG.RandFloatSigned() * a_Parameters.ActivationAMutationMaxPower;

      m_NeuronGenes[i].m_A += t_randnum;

      Clamp(m_NeuronGenes[i].m_A, a_Parameters.MinActivationA,
            a_Parameters.MaxActivationA);
    }
  }
...
bool Genome::Mutate_NeuronActivations_B(const Parameters &a_Parameters,
                                        RNG &a_RNG) {
  // for all neurons..
  for (unsigned int i = 0; i < NumNeurons(); i++) {
    // skip inputs and bias
    if ((m_NeuronGenes[i].Type() != INPUT) &&
        (m_NeuronGenes[i].Type() != BIAS)) {
      double t_randnum =
          a_RNG.RandFloatSigned() * a_Parameters.ActivationBMutationMaxPower;

      m_NeuronGenes[i].m_B += t_randnum;

      Clamp(m_NeuronGenes[i].m_B, a_Parameters.MinActivationB,
            a_Parameters.MaxActivationB);
    }
  }
...
bool Genome::Mutate_NeuronActivation_Type(const Parameters &a_Parameters,
                                          RNG &a_RNG) {
  ...
  m_NeuronGenes[t_choice].m_ActFunction =
      GetRandomActivation(a_Parameters, a_RNG);
...
bool Genome::Mutate_NeuronTimeConstants(const Parameters &a_Parameters,
                                        RNG &a_RNG) {
  // for all neurons..
  for (unsigned int i = 0; i < NumNeurons(); i++) {
    // skip inputs and bias
    if ((m_NeuronGenes[i].Type() != INPUT) &&
        (m_NeuronGenes[i].Type() != BIAS)) {
      double t_randnum =
          a_RNG.RandFloatSigned() * a_Parameters.TimeConstantMutationMaxPower;
...
bool Genome::Mutate_NeuronBiases(const Parameters &a_Parameters, RNG &a_RNG) {
  ...
  for (unsigned int i = 0; i < NumNeurons(); i++) {
    ...
    if ((m_NeuronGenes[i].Type() != INPUT) &&
        (m_NeuronGenes[i].Type() != BIAS)) {
      double t_randnum =
          a_RNG.RandFloatSigned() * a_Parameters.BiasMutationMaxPower;

      m_NeuronGenes[i].m_Bias += t_randnum;

      Clamp(m_NeuronGenes[i].m_TimeConstant, a_Parameters.MinNeuronBias,
            a_Parameters.MaxNeuronBias);
    }
  }
...
Genome Genome::Mate(Genome &a_Dad, bool a_MateAverage, bool a_InterSpecies,
  ...
  if (a_RNG.RandFloat() < 0.5) {
    // pick
    Gene n =
        ((a_RNG.RandFloat() < 0.5f) == 0) ? m_GenomeGene : a_Dad.m_GenomeGene;
  ...
  if (!a_Parameters.DontUseBiasNeuron) {
    ...
    for (i = 0; i < m_NumInputs - 1; i++) {
      ...
      if (a_RNG.RandFloat() < 0.5) {
        // pick
        NeuronGene n = ((a_RNG.RandFloat() < 0.5f) == 0)
                           ? m_NeuronGenes[i]
                           : a_Dad.m_NeuronGenes[i];
    ...
    if (a_RNG.RandFloat() < 0.5) {
      // the bias
      NeuronGene nb = ((a_RNG.RandFloat() < 0.5f) == 0)
                          ? m_NeuronGenes[i]
                          : a_Dad.m_NeuronGenes[i];
  ...
  for (unsigned int i = 0; i < m_NumOutputs; i++) {
    ...
    if (a_RNG.RandFloat() < 0.5) {
      ...
      // from mother
      t_tempneuron = GetNeuronByIndex(i + m_NumInputs);
      t_tempneuron.MateTraits(a_Dad.GetNeuronByIndex(i + m_NumInputs).m_Traits,
                              a_RNG);
    }
  ...
  while (!((t_curMom == m_LinkGenes.end()) &&
           (t_curDad == a_Dad.m_LinkGenes.end()))) {
      ...
      if (t_innov_mom == t_innov_dad) {
        ...
        if (!a_MateAverage) {
          ...
          const double t_Weight =
              (t_curDad->GetWeight() + t_curMom->GetWeight()) / 2.0;
    ...
    if ((t_selectedgene.InnovationID() > 0) &&
        (!t_baby.HasLink(t_selectedgene.FromNeuronID(),
...
unsigned int Genome::NeuronDepth(int a_NeuronID, unsigned int a_Depth) {
  ...
  // Base case
  if ((GetNeuronByID(a_NeuronID).Type() == INPUT) ||
      (GetNeuronByID(a_NeuronID).Type() == BIAS)) {
    return a_Depth;
  }
...
Genome::Genome(std::ifstream &a_DataFile) {
  ...
  do {
    ...
    if (t_Str == "Neuron") {
      ...
      t_neuron.Init(t_a, t_b, t_timeconst, t_bias,
                    static_cast<ActivationFunction>(t_activationfunc));
    ...
    if (t_Str == "Link") {
      ...
      m_LinkGenes.push_back(LinkGene(t_from, t_to, t_innov, t_weight,
                                     static_cast<bool>(t_isrecur)));
    }
...
void Genome::Save(FILE *a_file) {
  ...
  for (unsigned int i = 0; i < NumNeurons(); i++) {
    ...
    fprintf(a_file, "Neuron %d %d %3.8f %d %3.8f %3.8f %3.8f %3.8f\n",
            ...
            m_NeuronGenes[i].SplitY(),
            static_cast<int>(m_NeuronGenes[i].m_ActFunction),
            m_NeuronGenes[i].m_A, m_NeuronGenes[i].m_B,
  ...
  for (unsigned int i = 0; i < NumLinks(); i++) {
    fprintf(a_file, "Link %d %d %d %d %3.8f\n", m_LinkGenes[i].FromNeuronID(),
            m_LinkGenes[i].ToNeuronID(), m_LinkGenes[i].InnovationID(),
            static_cast<int>(m_LinkGenes[i].IsRecurrent()),
...
You can use any subsstrate, but the hidden nodes in it will not be used for the
generation. Relies on the Divide Initialize, PruneExpress and CleanNet methods.
*/

void Genome::BuildESHyperNEATPhenotype(NeuralNetwork &net, Substrate &subst,
                                       Parameters &params) {
  ASSERT(subst.m_input_coords.size() > 0);
  ASSERT(subst.m_output_coords.size() > 0);
  ...
  for (unsigned int i = 0; i < input_count; i++) {
    ...
    root = boost::shared_ptr<QuadPoint>(new QuadPoint(
        params.Qtree_X, params.Qtree_Y, params.Width, params.Height, 1));
    DivideInitialize(subst.m_input_coords[i], root, t_temp_phenotype, params,
                     true, 0.0);
    TempConnections.clear();
    PruneExpress(subst.m_input_coords[i], root, t_temp_phenotype, params,
                 TempConnections, true);
    ...
    for (unsigned int j = 0; j < TempConnections.size(); j++) {
      ...
      if (hidden_nodes.find(TempConnections[j].target) == hidden_nodes.end()) {
        ...
        hidden_nodes.insert(
            std::make_pair(TempConnections[j].target, target_index));
      }
      // Add connection
      else {
  ...
  // Hidden to hidden.
  // Basically the same procedure as above repeated IterationLevel times (see
  // the params)
  ...
  for (unsigned int i = 0; i < params.IterationLevel; i++) {
    ...
    for (itr_hid = unexplored_nodes.begin(); itr_hid != unexplored_nodes.end();
         itr_hid++) {
      root = boost::shared_ptr<QuadPoint>(new QuadPoint(
          params.Qtree_X, params.Qtree_Y, params.Width, params.Height, 1));
      DivideInitialize(itr_hid->first, root, t_temp_phenotype, params, true,
                       0.0);
      TempConnections.clear();
      PruneExpress(itr_hid->first, root, t_temp_phenotype, params,
                   TempConnections, true);
      // root.reset();
      ...
      for (unsigned int k = 0; k < TempConnections.size(); k++) {
        ...
        if (hidden_nodes.find(TempConnections[k].target) ==
            hidden_nodes.end()) {
          target_index = hidden_counter++;
          hidden_nodes.insert(
              std::make_pair(TempConnections[k].target, target_index));
  ...
  // Finally Output to Hidden. Note that unlike before, here we connect the
  // outputs to existing hidden nodes and no new nodes are added.
  for (unsigned int i = 0; i < output_count; i++) {
    root = boost::shared_ptr<QuadPoint>(new QuadPoint(
        params.Qtree_X, params.Qtree_Y, params.Width, params.Height, 1));
    DivideInitialize(subst.m_output_coords[i], root, t_temp_phenotype, params,
                     false, 0.0);
    TempConnections.clear();
    PruneExpress(subst.m_output_coords[i], root, t_temp_phenotype, params,
                 TempConnections, false);
  ...
  // Add the neurons.Input first, followed by bias, output and hidden. In this
  // order.
...
void Genome::DivideInitialize(
    const std::vector<double> &node, boost::shared_ptr<QuadPoint> &root,
    NeuralNetwork &cppn, Parameters &params, const bool &outgoing,
    const double
        &z_coord) { // Have to check if this actually does something useful here
  // CalculateDepth();
  ...
  // and if they have higher variance add them to their parent. Repeat with the
  // children until maxDepth has been reached or if the variance isn't high
  // enough.
  ...
  while (!q.empty()) {
    ...
    p->children.push_back(boost::shared_ptr<QuadPoint>(
        new QuadPoint(p->x - p->width / 2, p->y - p->height / 2, p->width / 2,
                      p->height / 2, p->level + 1)));
    p->children.push_back(boost::shared_ptr<QuadPoint>(
        new QuadPoint(p->x - p->width / 2, p->y + p->height / 2, p->width / 2,
                      p->height / 2, p->level + 1)));
    p->children.push_back(boost::shared_ptr<QuadPoint>(
    ...
    if ((p->level < params.InitialDepth) ||
        ((p->level < params.MaxDepth) &&
         Variance(p) > params.DivisionThreshold)) {
...
// We take the tree generated above and see which connections can be expressed
// on the basis of Variance threshold, Band threshold and LEO.
  ...
  if (root->children[0] == NULL) {
    ...
    for (unsigned int i = 0; i < 4; i++) {
      if (Variance(root->children[i]) > params.VarianceThreshold) {
        PruneExpress(node, root->children[i], cppn, params, connections,
                     outgoing);
      }

      // Band Pruning phase.
      // If LEO is turned off this should always happen.
      // If it is not it should only happen if the LEO output is greater than a
      // specified threshold
      else if (!params.Leo ||
               (params.Leo && root->children[i]->leo > params.LeoThreshold)) {
        // CalculateDepth();
        int cppn_depth = 8; // GetDepth();
        ...
        if (std::max(std::min(d_top, d_bottom), std::min(d_left, d_right)) >
...
void Genome::CollectValues(std::vector<double> &vals,
...
void Genome::Clean_Net(std::vector<Connection> &connections,
  ...
  while (loose_connections) {
    ...
    for (unsigned int i = 0; i < connections.size(); i++) {
      if (connections[i].m_source_neuron_idx !=
          connections[i].m_target_neuron_idx) {
    ...
    for (itr = connections.begin(); itr < connections.end();) {
      if (!hasOutgoing[itr->m_target_neuron_idx] ||
          !hasIncoming[itr->m_source_neuron_idx]) {
