...
#include <MultiNEAT/Population.hh>
#include <MultiNEAT/Random.hh>
#include <MultiNEAT/Species.hh>
...
Population::Population(const Genome &a_Seed, const Parameters &a_Parameters,
                       bool a_RandomizeWeights, double a_RandomizationRange,
                       int a_RNG_seed) {
      ...
      while (is_invalid) {
        ...
        if (!m_Parameters.AllowClones) {
          for (unsigned int j = 0; j < m_Genomes.size(); j++) {
            ...
            {
              if (m_Genomes[i].CompatibilityDistance(
                      m_Genomes[j], m_Parameters) < 0.000001) // equal genomes?
              {
                is_invalid = true;
                break;
              }
...
void Population::Speciate() {
  ...
  // NOTE: we are comparing the new generation's genomes to the representatives
  // from the previous generation! Any new species that is created is assigned a
  // representative from the new generation.
  for (unsigned int i = 0; i < m_Genomes.size(); i++) {
    t_added = false;

    // iterate through each species and check if compatible. If compatible, then
    // add to the species. if not compatible, create a new species.
...
void Population::CountOffspring() {
  ...
  t_average_adjusted_fitness = t_total_adjusted_fitness /
                               static_cast<double>(m_Parameters.PopulationSize);

  // Calculate how much offspring each individual should have
  for (unsigned int i = 0; i < m_Species.size(); i++) {
    for (unsigned int j = 0; j < m_Species[i].m_Individuals.size(); j++) {
      m_Species[i].m_Individuals[j].SetOffspringAmount(
          m_Species[i].m_Individuals[j].GetAdjFitness() /
...
void Population::Epoch() {
  ...
  // Incrementing the global stagnation counter, we can check later for global
  // stagnation
  ...
  for (unsigned int i = 0; i < m_Species.size(); i++) {
    ...
    for (unsigned int j = 0; j < m_Species[i].m_Individuals.size(); j++) {
      ...
      if (m_BestFitnessEver < t_Fitness) {
        // Reset the stagnation counter only if the fitness jump is greater or
        // equal to the delta.
        if (fabs(t_Fitness - m_BestFitnessEver) >=
            m_Parameters.StagnationDelta) {
          m_GensSinceBestFitnessLastChanged = 0;
        }
  ...
  if (m_Parameters.DynamicCompatibility == true) {
    if ((m_Generation % m_Parameters.CompatTreshChangeInterval_Generations) ==
        0) {
    ...
    if (m_Parameters.CompatThreshold < m_Parameters.MinCompatThreshold)
      m_Parameters.CompatThreshold = m_Parameters.MinCompatThreshold;
  }
  ...
  if (m_Parameters.DeltaCoding) {
    if (m_GensSinceBestFitnessLastChanged >
        (m_Parameters.SpeciesMaxStagnation + 10)) {
  ...
  if (m_Parameters.PhasedSearching) {
    ...
    if (m_SearchMode == COMPLEXIFYING) {
      ...
      if (m_CurrentMPC >
          (m_BaseMPC + m_Parameters.SimplifyingPhaseMPCThreshold)) {
        // Do this only if the whole population is stagnating
        if (m_GensSinceBestFitnessLastChanged >
            m_Parameters.SimplifyingPhaseStagnationThreshold) {
  ...
  // for(unsigned int i=0; i<m_Species.size(); i++)
  // m_Species[i].KillWorst(m_Parameters);
  ...
  for (unsigned int i = 0; i < m_Species.size(); i++)
    t_total_genomes +=
        static_cast<unsigned int>(m_Species[i].m_Individuals.size());
...
unsigned int Population::ChooseParentSpecies() {
  ...
  } while (m_Species[t_curspecies].m_AverageFitness ==
           0); // prevent species with no evaluated members to be chosen
...
// Main realtime loop. We assume that the whole population was evaluated once
// before calling this. Returns a pointer to the baby in the population. It will
// be the only individual that was not evaluated. Set the m_Evaluated flag of
// the baby to true after evaluation!
Genome *Population::Tick(Genome &a_deleted_genome) {
  ...
  for (unsigned int i = 0; i < m_Species.size(); i++) {
    for (unsigned int j = 0; j < m_Species[i].m_Individuals.size(); j++) {
      ...
      if (m_Species[i].m_Individuals[j].GetFitness() >=
          m_Species[i].GetBestFitness()) {
  ...
  // If the compatibility treshold was changed, reassign all individuals by
  // species
  if (t_changed) {
    for (unsigned int i = 0; i < m_Genomes.size(); i++) {
      ReassignSpecies(i);
  ...
  Genome t_baby = m_Species[t_parent_species_index].ReproduceOne(
      *this, m_Parameters, m_RNG);
  ...
  if (t_cur_species == m_Species.end()) {
    ...
    // the last one
    t_to_return =
        &(m_Species[m_Species.size() - 1].m_Individuals
              [m_Species[m_Species.size() - 1].m_Individuals.size() - 1]);
    IncrementNextSpeciesID();
  } else {
    // try to find a compatible species
    Genome t_to_compare = t_cur_species->GetRepresentative();
    ...
    while ((t_cur_species != m_Species.end()) && (!t_found)) {
      if (t_baby.IsCompatibleWith(t_to_compare, m_Parameters)) {
        ...
        t_to_return =
            &(t_cur_species
                  ->m_Individuals[t_cur_species->m_Individuals.size() - 1]);
        t_found = true; // the search is over
      } else {
        // keep searching for a matching species
        t_cur_species++;
...
// Warning! All derived classes MUST NOT have any member variables! Change the
// algorithms only!
void Population::InitPhenotypeBehaviorData(
  ...
  int counter = 0;
  for (unsigned int i = 0; i < m_Species.size(); i++) {
    for (unsigned int j = 0; j < m_Species[i].m_Individuals.size();
         j++, counter++) {
      m_Species[i].m_Individuals[j].m_PhenotypeBehavior =
          &((*a_population)[counter]);
...
double Population::ComputeSparseness(Genome &genome) {
  ...
  for (unsigned int i = 0; i < m_Species.size(); i++) {
    for (unsigned int j = 0; j < m_Species[i].m_Individuals.size(); j++) {
      double distance = genome.m_PhenotypeBehavior->Distance_To(
          m_Species[i].m_Individuals[j].m_PhenotypeBehavior);
  ...
  for (unsigned int i = 0; i < m_BehaviorArchive->size(); i++) {
    t_distances_list.push_back(
        genome.m_PhenotypeBehavior->Distance_To(&((*m_BehaviorArchive)[i])));
  }
...
bool Population::NoveltySearchTick(Genome &a_SuccessfulGenome) {
  ...
  if ((m_NumEvaluations %
    ...
    for (unsigned int i = 0; i < m_Species.size(); i++) {
      for (unsigned int j = 0; j < m_Species[i].m_Individuals.size(); j++) {
        m_Species[i].m_Individuals[j].SetFitness(
            ComputeSparseness(m_Species[i].m_Individuals[j]));
      }
    }
  }
  ...
  if (t_sparseness > m_Parameters.NoveltySearch_P_min) {
    ...
    /*for(unsigned int i=0; i<(*m_BehaviorArchive).size(); i++)
    {
        if ( (*(t_new_baby->m_PhenotypeBehavior)).m_Data ==
    (*m_BehaviorArchive)[i].m_Data )
        {
            present = true;
            break;
        }
  ...
  if (m_Parameters.NoveltySearch_Dynamic_Pmin) {
    ...
    if (m_GensSinceLastArchiving >
        m_Parameters.NoveltySearch_No_Archiving_Stagnation_Threshold) {
      m_Parameters.NoveltySearch_P_min *=
          m_Parameters.NoveltySearch_Pmin_lowering_multiplier;
      if (m_Parameters.NoveltySearch_P_min <
          m_Parameters.NoveltySearch_Pmin_min) {
        m_Parameters.NoveltySearch_P_min = m_Parameters.NoveltySearch_Pmin_min;
      }
    }

    // too much additions to the archive (one after another)?
    if (m_QuickAddCounter >
        m_Parameters.NoveltySearch_Quick_Archiving_Min_Evaluations) {
      m_Parameters.NoveltySearch_P_min *=
          m_Parameters.NoveltySearch_Pmin_raising_multiplier;
    }
  }
