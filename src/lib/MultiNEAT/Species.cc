...
Genome Species::GetIndividual(Parameters &a_Parameters, RNG &a_RNG) const {
  ...
  if (!a_Parameters.RouletteWheelSelection) { // start with the last one just
                                              // for comparison sake
    // int temp_genome;

    // int t_num_parents = static_cast<int>( floor(
    //        (a_Parameters.SurvivalRate *
    //        (static_cast<double>(t_Evaluated.size()))) + 1.0));
    int t_num_parents =
        (int)(a_Parameters.SurvivalRate * (double)(t_Evaluated.size()));
    ...
    /*for (unsigned int i = 0; i < a_Parameters.TournamentSize; i++)
    {
        ...
        if (m_Individuals[temp_genome].GetFitness() >
    m_Individuals[t_chosen_one].GetFitness())
        {
...
Genome Species::GetRandomIndividual(RNG &a_RNG) const {
  if (m_Individuals.size() == 0) // no members yet, return representative
  {
    ...
    int t_rand_choice = 0;
    t_rand_choice =
        a_RNG.RandInt(0, static_cast<int>(m_Individuals.size() - 1));
...
Genome Species::GetRepresentative() const { return m_Representative; }
...
void Species::Reproduce(Population &a_Pop, Parameters &a_Parameters,
  ...
  int elite_offspring =
      Rounded(a_Parameters.EliteFraction * m_Individuals.size());
  ...
  while (t_offspring_count--) {
    ...
    if (elite_count < elite_offspring) {
      ...
      do // - while the baby already exists somewhere in the new population or
         // turned invalid in some way
      {
        ...
        // else we can mate
          ...
          if ((a_RNG.RandFloat() < a_Parameters.CrossoverRate) &&
              (a_Pop.GetSearchMode() != SIMPLIFYING)) {
            // get the father
            Genome t_dad;
            bool t_interspecies = false;

            // There is a probability that the father may come from another
            // species
            if ((a_RNG.RandFloat() < a_Parameters.InterspeciesCrossoverRate) &&
                (a_Pop.m_Species.size() > 1)) {
              // Find different species (random one) // !!!!!!!!!!!!!!!!!
              int t_diffspec = a_RNG.RandInt(
                  0, static_cast<int>(a_Pop.m_Species.size() - 1));
            ...
            if (a_RNG.RandFloat() < a_Parameters.MultipointCrossoverRate) {
              t_baby =
                  t_mom.Mate(t_dad, false, t_interspecies, a_RNG, a_Parameters);
            } else {
              t_baby =
                  t_mom.Mate(t_dad, true, t_interspecies, a_RNG, a_Parameters);
            }
        ...
        // Mutate the baby
        if ((!t_mated) ||
            (a_RNG.RandFloat() < a_Parameters.OverallMutationRate)) {
          MutateGenome(t_baby_exists_in_pop, a_Pop, t_baby, a_Parameters,
                       a_RNG);
        }
        ...
        if (!a_Parameters.AllowClones) {
          for (unsigned int i = 0; i < a_Pop.m_TempSpecies.size(); i++) {
            for (unsigned int j = 0;
                 j < a_Pop.m_TempSpecies[i].m_Individuals.size(); j++) {
              if ((t_baby.CompatibilityDistance(
                       a_Pop.m_TempSpecies[i].m_Individuals[j],
                       a_Parameters) <
                   COMPAT_EQUALITY_DELTA) // identical genome?
              ) {
          ...
          for (unsigned int i = 0; i < a_Pop.m_GenomeArchive.size(); i++) {
            if ((t_baby.CompatibilityDistance(a_Pop.m_GenomeArchive[i],
              ...
              break;
               ...
               (t_baby.FailsConstraints(a_Parameters))); // end do
    }
    ...
    // after all reproduction completes, the original species will be replaced
    // back
    ...
    if (t_cur_species == a_Pop.m_TempSpecies.end()) {
      ...
      if (!t_found) {
        a_Pop.m_TempSpecies.push_back(
            Species(t_baby, a_Pop.GetNextSpeciesID()));
        a_Pop.IncrementNextSpeciesID();
      }
    }
  }
  ...
  do // - while the baby turned invalid in some way
  {
    ...
    // else we can mate
      ...
      if ((a_RNG.RandFloat() < a_Parameters.CrossoverRate) &&
        ...
        if ((a_RNG.RandFloat() < a_Parameters.InterspeciesCrossoverRate) &&
          ...
          // Find different species (random one) // !!!!!!!!!!!!!!!!!
          int t_diffspec =
              a_RNG.RandInt(0, static_cast<int>(a_Pop.m_Species.size() - 1));
          t_dad =
              a_Pop.m_Species[t_diffspec].GetIndividual(a_Parameters, a_RNG);
          t_interspecies = true;
        } else {
          // Mate within species
          t_dad = GetIndividual(a_Parameters, a_RNG);
    ...
    if (!a_Parameters.AllowClones) {
      for (unsigned int i = 0; i < a_Pop.m_Species.size(); i++) {
        for (unsigned int j = 0; j < a_Pop.m_Species[i].m_Individuals.size();
             j++) {
...
// Mutates a genome
void Species::MutateGenome(bool t_baby_is_clone, Population &a_Pop,
                           Genome &t_baby, Parameters &a_Parameters,
                           RNG &a_RNG) {
#if 0
...
#else
  // We will perform roulette wheel selection to choose the type of mutation and
  // will mutate the baby This method guarantees that the baby will be mutated
  ...
  // Special consideration for phased searching - do not allow certain mutations
  // depending on the search mode also don't use additive mutations if we just
  // want to get rid of the clones
  ...
  while (t_mutation_success == false) {
    ...
    case ADD_NODE:
      t_mutation_success = t_baby.Mutate_AddNeuron(
          a_Pop.AccessInnovationDatabase(), a_Parameters, a_RNG);
      break;

    case ADD_LINK:
      t_mutation_success = t_baby.Mutate_AddLink(
          a_Pop.AccessInnovationDatabase(), a_Parameters, a_RNG);
      break;

    case REMOVE_NODE:
      t_mutation_success = t_baby.Mutate_RemoveSimpleNeuron(
          a_Pop.AccessInnovationDatabase(), a_RNG);
      break;
