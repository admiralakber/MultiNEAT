...
public:
    ...
    for (auto it = tp.begin(); it != tp.end(); it++) {
      ...
      if (it->second.type == "int") {
        IntTraitParameters itp =
            bs::get<IntTraitParameters>(it->second.m_Details);
        t = a_RNG.RandInt(itp.min, itp.max);
      }
      if (it->second.type == "float") {
        FloatTraitParameters itp =
            bs::get<FloatTraitParameters>(it->second.m_Details);
      ...
      if (it->second.type == "str") {
        StringTraitParameters itp =
            bs::get<StringTraitParameters>(it->second.m_Details);
      ...
      if (it->second.type == "intset") {
        IntSetTraitParameters itp =
            bs::get<IntSetTraitParameters>(it->second.m_Details);
        std::vector<double> probs = itp.probs;
        if (itp.set.size() == 0) {
      ...
      if (it->second.type == "floatset") {
        FloatSetTraitParameters itp =
            bs::get<FloatSetTraitParameters>(it->second.m_Details);
        std::vector<double> probs = itp.probs;
        if (itp.set.size() == 0) {
  ...
  void MateTraits(const std::map<std::string, Trait> &t, RNG &a_RNG) {
    for (auto it = t.begin(); it != t.end(); it++) {
      ...
      if (!(mine.type() == yours.type())) {
        throw std::runtime_error("Types of traits doesn't mat.hh");
      }
  ...
  bool MutateTraits(const std::map<std::string, TraitParameters> &tp,
                    RNG &a_RNG) {
      ...
      if (it->second.dep_key != "") {
        ...
        if (m_Traits.count(it->second.dep_key) != 0) {
          ...
          for (int ix = 0; ix < it->second.dep_values.size(); ix++) {
            if (m_Traits[it->second.dep_key].value ==
                it->second.dep_values[ix]) {
              doit = true;
              break;
            }
          }
      ...
      if (doit) {
        if (it->second.type == "int") {
          ...
          if (a_RNG.RandFloat() < it->second.m_MutationProb) {
            // determine type of mutation - modify or replace, according to
            // parameters
        ...
        if (it->second.type == "floatset") {
          ...
          int idx = a_RNG.Roulette(probs);
          floatsetelement cur =
              bs::get<floatsetelement>(m_Traits[it->first].value);
  ...
  GetTraitDistances(const std::map<std::string, Trait> &other) {
    ...
    for (auto it = other.begin(); it != other.end(); it++) {
      ...
      if (!(mine.type() == yours.type())) {
        throw std::runtime_error("Types of traits don't mat.hh");
      }
      ...
      if (it->second.dep_key != "") {
        ...
        if (m_Traits.count(it->second.dep_key) != 0) {
          ...
          for (int ix = 0; ix < it->second.dep_values.size(); ix++) {
            if ((m_Traits[it->second.dep_key].value ==
                 it->second.dep_values[ix]) &&
                (other.at(it->second.dep_key).value ==
                 it->second.dep_values[ix])) {
              doit = true;
              break;
            }
      ...
      if (doit) {
        ...
        if (mine.type() == typeid(intsetelement)) {
          // distance between ints - calculate directly
          dist[it->first] = abs((bs::get<intsetelement>(mine)).value -
                                (bs::get<intsetelement>(yours)).value);
        }
        if (mine.type() == typeid(floatsetelement)) {
          // distance between floats - calculate directly
          dist[it->first] = abs((bs::get<floatsetelement>(mine)).value -
                                (bs::get<floatsetelement>(yours)).value);
        }
      }
    }
...
public:
  ...
  LinkGene(int a_InID, int a_OutID, int a_InnovID, double a_Wgt,
           bool a_Recurrent = false) {
  ...
  // overload '<', '>', '!=' and '==' used for sorting and comparison (we use
  // the innovation ID as the criteria)
...
public:
  ...
  friend bool operator==(const NeuronGene &a_lhs, const NeuronGene &a_rhs) {
    return (a_lhs.m_ID == a_rhs.m_ID) && (a_lhs.m_Type == a_rhs.m_Type) &&
           (a_lhs.m_SplitY == a_rhs.m_SplitY) && (a_lhs.m_A == a_rhs.m_A) &&
           (a_lhs.m_B == a_rhs.m_B) &&
  ...
  // Initializing
  void Init(double a_A, double a_B, double a_TimeConstant, double a_Bias,
            ActivationFunction a_ActFunc) {
    m_A = a_A;
    m_B = a_B;
