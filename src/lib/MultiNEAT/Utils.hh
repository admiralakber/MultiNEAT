...
inline void GetMaxMin(const std::vector<double> &a_Vals, double &a_Min,
                      double &a_Max) {
  a_Max = std::numeric_limits<double>::min();
  a_Min = std::numeric_limits<double>::max();
  for (std::vector<double>::const_iterator t_It = a_Vals.begin();
       t_It != a_Vals.end(); ++t_It) {
...
inline int RoundUnderOffset(const double a_Val, const double a_Offset) {
  // ASSERT(a_Offset < 1 && a_Offset > -1); ???!? Should this be a test for the
  // offset
  const int t_Integral = static_cast<int>(a_Val);
  const double t_Mantissa = a_Val - t_Integral;
...
// Scales the value "a", that is in range [a_min .. a_max] into its relative
// value in the range [tr_min .. tr_max] Example: A=2, in the range [0 .. 4] ..
// we want to scale it to the range [-12 .. 12] .. we get 0..
inline void Scale(double &a, const double a_min, const double a_max,
                  const double a_tr_min, const double a_tr_max) {
...
// value in the range [tr_min .. tr_max] Example: A=2, in the range [0 .. 4] ..
// we want to scale it to the range [-12 .. 12] .. we get 0..
inline void Scale(float &a, const double a_min, const double a_max,
                  const double a_tr_min, const double a_tr_max) {
  //        ASSERT((a >= a_min) && (a <= a_max));
  //        ASSERT(a_min <= a_max);
  //        ASSERT(a_tr_min <= a_tr_max);
