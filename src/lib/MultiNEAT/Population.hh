...
public:
  ...
  // Initializes a population from a seed genome G. Then it initializes all
  // weights To small numbers between -R and R. The population size is
  // determined by GlobalParameters.PopulationSize
  ...
  unsigned int GetStagnation() const {
    return m_GensSinceBestFitnessLastChanged;
  }
  ...
  InnovationDatabase &AccessInnovationDatabase() {
    return m_InnovationDatabase;
  }
  ...
  // Removes worst member of the whole population that has been around for a
  // minimum amount of time returns the genome that was just deleted (may be
  // useful)
  Genome RemoveWorstIndividual();

  // The main reaitime tick. Analog to Epoch(). Replaces the worst evaluated
  // individual with a new one. Returns a pointer to the new baby. and copies
  // the genome that was deleted to a_geleted_genome
