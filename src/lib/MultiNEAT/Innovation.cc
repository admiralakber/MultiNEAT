...
///////////////////////////////////////////////////////////////////////////////
// File:        Innovation.cpp
// Description: Implementation for the Innovation and InnovationDatabase
// classes.
///////////////////////////////////////////////////////////////////////////////
...
// Creates an empty database but this time sets the next innov number and neuron
// ID
InnovationDatabase::InnovationDatabase(int a_LastInnovationNum,
                                       int a_LastNeuronID) {
...
void InnovationDatabase::Init(const Genome &a_Genome) {
  ...
  for (unsigned int i = 0; i < a_Genome.NumLinks(); i++) {
    Innovation t_innov(a_Genome.GetLinkByIndex(i).InnovationID(), NEW_LINK,
                       a_Genome.GetLinkByIndex(i).FromNeuronID(),
...
void InnovationDatabase::Init(std::ifstream &a_DataFile) {
  ...
  do {
    ...
    if (t_str == "Innovation") {
      ...
      m_Innovations.push_back(
          Innovation(t_id, static_cast<InnovationType>(t_innovtype), t_from,
                     t_to, static_cast<NeuronType>(t_neurontype), t_nid));
    }
...
void InnovationDatabase::Save(FILE *a_file) {
  ...
  for (unsigned int i = 0; i < m_Innovations.size(); i++) {
    fprintf(a_file, "Innovation %d %d %d %d %d %d\n", m_Innovations[i].ID(),
            static_cast<int>(m_Innovations[i].InnovType()),
            m_Innovations[i].FromNeuronID(), m_Innovations[i].ToNeuronID(),
            static_cast<int>(m_Innovations[i].GetNeuronType()),
...
// If it is a NEW_LINK innovation, in & out specify the neuron IDs being
// connected If it is a NEW_NEURON innovation, in & out specify the connection
// that was split
int InnovationDatabase::CheckInnovation(int a_In, int a_Out,
                                        InnovationType a_Type) const {
  ASSERT((a_In > 0) && (a_Out > 0));
  ASSERT((a_Type == NEW_NEURON) || (a_Type == NEW_LINK));

  // search the list for a match
  for (unsigned int i = 0; i < m_Innovations.size(); i++) {
    if ((m_Innovations[i].FromNeuronID() == a_In) &&
        (m_Innovations[i].ToNeuronID() == a_Out) &&
        (m_Innovations[i].InnovType() == a_Type)) {
      // match found?
...
int InnovationDatabase::CheckLastInnovation(int a_In, int a_Out,
                                            InnovationType a_Type) const {
  ASSERT((a_In > 0) && (a_Out > 0));
  ASSERT((a_Type == NEW_NEURON) || (a_Type == NEW_LINK));
  int t_ID = -1;
...
std::vector<int>
InnovationDatabase::CheckAllInnovations(int a_In, int a_Out,
                                        InnovationType a_Type) const {
  ASSERT((a_In > 0) && (a_Out > 0));
  ASSERT((a_Type == NEW_NEURON) || (a_Type == NEW_LINK));
...
int InnovationDatabase::FindNeuronID(int a_In, int a_Out) const {
  ...
  for (unsigned int i = 0; i < m_Innovations.size(); i++) {
    if ((m_Innovations[i].FromNeuronID() == a_In) &&
        (m_Innovations[i].ToNeuronID() == a_Out) &&
        (m_Innovations[i].InnovType() == NEW_NEURON)) {
      // match found?
...
int InnovationDatabase::AddLinkInnovation(int a_In, int a_Out) {
  ...
  m_Innovations.push_back(
      Innovation(m_NextInnovationNum, NEW_LINK, a_In, a_Out, NONE, -1));
  m_NextInnovationNum++;
...
int InnovationDatabase::AddNeuronInnovation(int a_In, int a_Out,
                                            NeuronType a_NType) {
  ASSERT((a_In > 0) && (a_Out > 0));
  ASSERT(!((a_NType == INPUT) || (a_NType == BIAS) || (a_NType == OUTPUT)));

  m_Innovations.push_back(Innovation(m_NextInnovationNum, NEW_NEURON, a_In,
                                     a_Out, a_NType, m_NextNeuronID));
  m_NextInnovationNum++;
  m_NextNeuronID++;
